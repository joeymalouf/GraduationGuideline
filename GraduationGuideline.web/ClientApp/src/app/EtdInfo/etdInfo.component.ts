...
@Component({
  selector: 'app-etdInfo',
  templateUrl: './etdInfo.component.html',
  styleUrls: ['./etdInfo.component.css']
})

@Injectable()
export class EtdInfoComponent {
  public step: Step;
  deadline: any;
  ...
  getStatus() {
    this.http.get<Step>('api/step/GetStep/ETD Info', {
  ...
  toggleStatus() {
    this.http.post<Step>('api/step/ToggleStepStatus/ETD Info', {}, {
      headers: new HttpHeaders({
        "Authorization": "Bearer " + this.token,
        "Content-Type": "application/json"
      })
