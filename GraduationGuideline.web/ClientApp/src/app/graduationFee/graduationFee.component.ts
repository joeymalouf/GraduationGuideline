...
@Component({
  selector: 'app-graduationFee',
  templateUrl: './graduationFee.component.html',
  styleUrls: ['./graduationFee.component.css']
})

@Injectable()
export class GraduationFeeComponent {
  public step: Step;
  deadline: any;
  ...
  getStatus() {
    this.http.get<Step>('api/step/GetStep/Graduation Fee', {
  ...
  toggleStatus() {
    this.http.post<Step>('api/step/ToggleStepStatus/Graduation Fee', {}, {
      headers: new HttpHeaders({
        "Authorization": "Bearer " + this.token,
        "Content-Type": "application/json"
      })
